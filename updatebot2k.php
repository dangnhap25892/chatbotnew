...
if ( !isUserExist($ID) ) { // nếu chưa tồn tại thì update lên sever
    $sql = "INSERT INTO `users` (`ID`, `trangthai`, `hangcho` ,`gioitinh`,`chatfuel`,`token`) VALUES (".$ID.", 0, 3 , $gioitinh,'$chatfuel','$token')";
   $info = mysqli_query($conn,$sql );
  header("Location: thamgiabot2k.php?ID=$ID&token=$token");
  }
header("Location: thamgiabot2k.php?ID=$ID&token=$token");
#mysqli_close($conn);
